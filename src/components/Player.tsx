...
export default function Player() {
  ...
  const handleSeek = useCallback((clientX: number, currentTarget: HTMLElement) => {
  ...
  return (
    <div className="w-full max-w-3xl">
      ...
      <div className="glass group relative flex flex-col items-center gap-3 rounded-3xl p-4 sm:flex-row sm:items-center sm:gap-6 sm:rounded-full sm:p-5 sm:pr-8">
        {/* Album Art */}
          ...
          >
            {currentSong.coverUrl ? (
              ...
              <div className="grid h-full w-full place-items-center bg-gradient-to-br from-[#00a1e0] via-[#0070d2] to-[#00509e]">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="white" className="opacity-90 sm:hidden">
                  <path d="M12 3v10.55A4 4 0 1014 17V7h4V3z" />
                </svg>
                <svg width="40" height="40" viewBox="0 0 24 24" fill="white" className="opacity-90 hidden sm:block">
                  <path d="M12 3v10.55A4 4 0 1014 17V7h4V3z" />
                </svg>
              </div>
            )}
          </div>
          <div className="pointer-events-none absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-black/70 ring-2 ring-white/30 sm:h-5 sm:w-5 sm:ring-3" />
        </div>

        {/* Track Info + Seek + Controls - full width on mobile */}
        <div className="flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-1 sm:items-stretch sm:gap-0">
          {/* Track Info */}
          <div className="min-w-0 w-full text-center sm:text-left">
            <p className="truncate text-base font-bold text-white drop-shadow-sm sm:text-lg">{currentSong.title}</p>
            <p className="truncate text-xs text-white/60 sm:text-sm">{currentSong.artist || currentSong.album}</p>
          </div>

          {/* Seek Bar */}
          <div className="mt-1 w-full sm:mt-3">
            <div
              className="seek-bar group/bar relative h-3 w-full cursor-pointer touch-none"
              role="slider"
              aria-label="Seek"
              aria-valuemin={0}
              aria-valuemax={100}
              ...
              onTouchStart={(e) => handleSeek(e.touches[0].clientX, e.currentTarget)}
              onTouchMove={(e) => {
                e.preventDefault();
                handleSeek(e.touches[0].clientX, e.currentTarget);
              }}
            >
              <div className="progress-bar absolute inset-x-0 top-1/2 h-1.5 -translate-y-1/2 rounded-full sm:h-2">
                <div className="progress-bar-fill h-full rounded-full" style={{ width: `${progress * 100}%` }} />
              </div>
              <div
                className="seek-thumb absolute top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white shadow-lg sm:h-5 sm:w-5"
                style={{ left: `${progress * 100}%` }}
              />
            </div>
            <div className="mt-1.5 flex justify-between text-[10px] tabular-nums text-white/50 sm:text-xs">
          ...
          <div className="flex items-center justify-center gap-1.5 sm:gap-2">
            <button
              ...
              className={`grid h-8 w-8 place-items-center rounded-full transition active:scale-95 sm:h-10 sm:w-10 ${
                isShuffled
            ...
            <button
              ...
              onClick={playPrev}
              className="grid h-10 w-10 place-items-center rounded-full text-white/80 transition hover:bg-[#00a1e0]/20 hover:text-white active:scale-95 sm:h-12 sm:w-12"
            >
            ...
            <button
              ...
              className="grid h-12 w-12 place-items-center rounded-full bg-white text-black shadow-2xl transition hover:scale-105 active:scale-95 disabled:opacity-50 sm:h-16 sm:w-16"
            >
              {playerState.playing ? (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" className="sm:w-7 sm:h-7">
